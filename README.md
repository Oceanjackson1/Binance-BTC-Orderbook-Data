...
| **Order Book 深度历史** | CoinGlass API | 历史下载 | ±1/2/3/5/10% 聚合深度，1 小时一条，支持跨交易所 |
...
├── futures_binance/
│   ├── futures_binance_range1pct_1h.csv    ← ±1% 范围
│   ├── futures_binance_range2pct_1h.csv    ← ±2% 范围
│   ├── futures_binance_range3pct_1h.csv
│   ├── futures_binance_range5pct_1h.csv
│   └── futures_binance_range10pct_1h.csv
├── spot_binance/                            ← 同上结构
├── futures_aggregated/                      ← 跨交易所聚合
├── spot_aggregated/                         ← 跨交易所聚合
├── futures_binance_all_ranges_1h.csv        ← 合并文件（7,080 行）
├── spot_binance_all_ranges_1h.csv
├── futures_aggregated_all_ranges_1h.csv
└── spot_aggregated_all_ranges_1h.csv
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `timestamp_utc` | datetime | 数据时间（UTC），每小时一条 |
...
| CoinGlass 深度 | 1 小时 | ±X% 聚合 bid/ask | 流动性趋势、买卖力量对比、跨交易所深度 |
...
| **CoinGlass 免费套餐** | 最小间隔 1h，5m/1m 需升级；历史约 180 天 |
...
| `Upgrade plan` | CoinGlass 免费套餐不支持 5m/1m | 升级套餐或使用 1h 间隔 |
